    ...
    <table class="table table-striped table-hover">
      ...
      <?php
           $uadharno=$_SESSION['adharno'];
			...
			$sql = "SELECT COUNT(*) AS cntrows FROM cart where paymentstatus='YES' ";
            $result = mysqli_query($link,$sql);
            $fetchresult = mysqli_fetch_array($result);
            $allcount = $fetchresult['cntrows'];
	  
	  $flag=0;
         $sql="SELECT * FROM payment ORDER BY id ASC limit $rowone,".$rowperpage;
		 $result = mysqli_query($link,$sql);
         $sno = $rowone + 1;
            ...
            while($row = mysqli_fetch_array($result))
				{
					...
					$fadharno=$row['fadharno'];
					
              $sql1="SELECT * FROM farmer where adharno='$fadharno'";
		      $resulte = mysqli_query($link,$sql1);
			     ...
			     if($rowe = mysqli_fetch_array($resulte))
				 {
				  ...
				  <td>'.$row['address'].$row['city'].$row['zip'].'</td>
				  <td>'.$row['mobileno'].'</td>
